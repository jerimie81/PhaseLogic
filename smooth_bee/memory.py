...
def _kb_set(name: str, data: dict, tags: str = "smooth-bee") -> None:
...
def load_user_preferences() -> dict:
    ...
    return _kb_get("smooth-bee:user-preferences")
...
def update_user_preferences(brief: dict) -> None:
    ...
    """
    prefs = _kb_get("smooth-bee:user-preferences")
    ...
    _kb_set("smooth-bee:user-preferences", prefs, tags="smooth-bee:preferences")
...
def record_project_completion(project_name: str, summary: dict) -> None:
    ...
    history = _kb_get("smooth-bee:project-history")
    ...
    _kb_set("smooth-bee:project-history", history, tags="smooth-bee:history")


def get_past_projects_summary() -> list[dict]:
    """Brief list of past smooth-bee projects (name, date, file count)."""
    try:
        with _conn() as conn:
            rows = conn.execute(
                ...
                "WHERE p.build_system = 'smooth-bee' "
...
def log_agent_call(agent_name: str, duration_s: float, succeeded: bool) -> None:
    ...
    """
    stats = _kb_get("smooth-bee:agent-stats")
    ...
    stats[agent_name] = a
    _kb_set("smooth-bee:agent-stats", stats, tags="smooth-bee:performance")


def load_agent_stats() -> dict:
    """Return {agent_name: {avg_s, calls, failures}} for all tracked agents."""
    return _kb_get("smooth-bee:agent-stats")
    ...
    try:
        with _conn() as conn:
            conn.execute(
                ...
                (
                    _now(),
                    f"smooth-bee:{project_name}/{artifact_name}",
                    before[:2000],
                    after[:2000],
                    "interactive edit",
